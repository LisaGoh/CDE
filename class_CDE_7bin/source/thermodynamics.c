...
int thermodynamics_init(
  ...
  pba->h =  pba->H0*_c_/100000 ;

  pba->Omega0_g = pba->Omega_g_tilde*pow(0.7/pba->h,2);
  pba->Omega0_b = pba->Omega_b_tilde*pow(0.7/pba->h,2);
  pba->Omega0_cdm = pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_rho_cdm]/pow(pba->background_table[(pba->bt_size-1)*pba->bg_size+pba->index_bg_H],2);
  pba->omega_cdm = pba->Omega0_cdm*pow(pba->h,2);
  pba->Omega0_ur = pba->Omega_ur_tilde*pow(0.7/pba->h,2);
