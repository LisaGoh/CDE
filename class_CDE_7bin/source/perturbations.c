    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_cdm == _TRUE_) {
        ...
        /* The the initial condition for the CDM velocity divergence theta_cdm=0 still holds in the CQ model  */
  ...
  if (_scalars_) {
    ...
    */
    if (pba->has_scf == _TRUE_) {
      ...
      /* NEW */     
      if (ppt->gauge == synchronous){
        delta_rho_scf =  1./a2*ppw->pvecback[pba->index_bg_phi_prime_scf]*y[ppw->pv->index_pt_phi_prime_scf]
           + ppw->pvecback[pba->index_bg_dV_scf]*y[ppw->pv->index_pt_phi_scf];
        delta_p_scf = 1./a2*ppw->pvecback[pba->index_bg_phi_prime_scf]*y[ppw->pv->index_pt_phi_prime_scf]
           - ppw->pvecback[pba->index_bg_dV_scf]*y[ppw->pv->index_pt_phi_scf];
      }
      /* */
      else{
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_theta_scf == _TRUE_) {
      rho_plus_p_theta_scf = k*k/a2*ppw->pvecback[pba->index_bg_phi_prime_scf]*y[ppw->pv->index_pt_phi_scf];
...
int perturbations_print_variables(double tau,
  ...
  if (_scalars_) {
    ...
    /* NEW */
    /* I have removed the part of the code that converted the density contrasts and velocities from the synchronous to the Newtonian gauge before being saved in the output file \
    just because I want to know the quantities in the gauge I am working i.e. the synchronous one */
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->gauge == synchronous) {
        ...
        dy[pv->index_pt_delta_cdm] = -(y[pv->index_pt_theta_cdm]+metric_continuity)-pvecback[pba->index_bg_beta]*pow(6,0.5)*y[pv->index_pt_phi_prime_scf]
        		                           +pow(6,0.5)*pvecback[pba->index_bg_H]*pvecback[pba->index_bg_beta_prime]*y[pv->index_pt_phi_scf]; /* cdm density */

        dy[pv->index_pt_theta_cdm] = (-a_prime_over_a+pow(6,0.5)*pvecback[pba->index_bg_beta]*pvecback[pba->index_bg_phi_prime_scf])*y[pv->index_pt_theta_cdm] 
                                      - k2*pvecback[pba->index_bg_beta]*pow(6,0.5)*y[pv->index_pt_phi_scf]; /* cdm velocity */
      }
      /* */
    }
    ...
    if (pba->has_scf == _TRUE_) {
      ...
      dy[pv->index_pt_phi_prime_scf] =  - 2.*a_prime_over_a*y[pv->index_pt_phi_prime_scf]
        ...
        + pvecback[pba->index_bg_beta]*a2*pow(6,0.5)*y[pv->index_pt_delta_cdm]*ppw->pvecback[pba->index_bg_rho_cdm]
        - pvecback[pba->index_bg_beta_prime]*a2*pow(6,0.5)*ppw->pvecback[pba->index_bg_rho_cdm]*y[pv->index_pt_phi_scf]*pvecback[pba->index_bg_H]/pvecback[pba->index_bg_phi_prime_scf]; 
      /* */
    }
    /** - ---> interacting dark radiation */
    if (pba->has_idr == _TRUE_){
