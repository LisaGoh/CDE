...
int input_shooting(struct file_content * pfc,
  ...
  char * const target_namestrings[] = {"Omega_dcdmdr",
                                       "omega_dcdmdr",
                                       "Omega_ini_dcdm",
                                       "omega_ini_dcdm",
                                       "sigma8"};
  ...
  char * const unknown_namestrings[] = {"Omega_ini_dcdm",           /* unknown param for target 'Omega_dcdmd' */
  ...
  enum computation_stage target_cs[] = {cs_background,     /* computation stage for target 'Omega_dcdmdr' */
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  class_call(parser_read_double(pfc,"T_cmb",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"Omega_g_tilde",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_g_tilde",&param3,&flag3,errmsg),
             errmsg,
             errmsg);
  class_test(class_at_least_two_of_three(flag1,flag2,flag3),
             errmsg,
             "You can only enter one of 'T_cmb', 'Omega_g_tilde' or 'omega_g_tilde'.");
  /* Complete set of parameters
  ...
  if (class_none_of_three(flag1,flag2,flag3)){
    /* NEW */
    pba->Omega_g_tilde = (4.*sigma_B/_c_*pow(pba->T_cmb,4.))/(3.*_c_*_c_*1.e10*pow(0.7,2)/_Mpc_over_m_/_Mpc_over_m_/8./_PI_/_G_);
  }
  else {
    /* NEW */
    if (flag1 == _TRUE_){
      pba->Omega_g_tilde = (4.*sigma_B/_c_*pow(param1,4.))/(3.*_c_*_c_*1.e10*pow(0.7,2)/_Mpc_over_m_/_Mpc_over_m_/8./_PI_/_G_);
      pba->T_cmb=param1;
    }
    if (flag2 == _TRUE_){
      pba->Omega_g_tilde = param2;
      pba->T_cmb = pow(pba->Omega_g_tilde*(3.*_c_*_c_*1.e10*pow(0.7,2)/_Mpc_over_m_/_Mpc_over_m_/8./_PI_/_G_)/(4.*sigma_B/_c_),0.25);
    }
    if (flag3 == _TRUE_){
      pba->Omega_g_tilde = param3/pow(0.7,2);
      pba->T_cmb = pow(pba->Omega_g_tilde*(3.*_c_*_c_*1.e10*pow(0.7,2)/_Mpc_over_m_/_Mpc_over_m_/8./_PI_/_G_)/(4.*sigma_B/_c_),0.25);
    }
  }
  ...
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_b_tilde or omega_b, choose one");
  if (flag1 == _TRUE_)
    pba->Omega_b_tilde = param1;
  if (flag2 == _TRUE_)
    pba->Omega_b_tilde = param2/pow(0.7,2);
  ...
  if (class_none_of_three(flag1,flag2,flag3)) {
    pba->Omega_ur_tilde = 3.044*7./8.*pow(4./11.,4./3.)*pba->Omega_g_tilde;
  }
  else {
    if (flag1 == _TRUE_) {
      pba->Omega_ur_tilde = param1*7./8.*pow(4./11.,4./3.)*pba->Omega_g_tilde;
    }
    if (flag2 == _TRUE_) {
      pba->Omega_ur_tilde = param2;
    }
    if (flag3 == _TRUE_) {
      pba->Omega_ur_tilde = param3/pow(0.7,2);
    }
  }
  ...
  /** 4) Omega_0_cdm (CDM) */
  /* Read */
  class_call(parser_read_double(pfc,"Omega_cdm_tilde",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_cdm_tilde",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  /* Test */
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "You can only enter one of 'Omega_cdm_tilde' or 'omega_cdm_tilde'.");
  /* Complete set of parameters */
  if (flag1 == _TRUE_)
    pba->Omega_cdm_tilde = param1;
  if (flag2 == _TRUE_)
    pba->Omega_cdm_tilde = param2/pow(0.7,2);
  /**/
  ...
  if (flag2 == _TRUE_){
    Omega_m_remaining = param2/pow(0.7,2);
    has_m_budget = _TRUE_;
  }
  ...
  if ((ppt->gauge == synchronous) && (pba->Omega_cdm_tilde==0)) pba->Omega_cdm_tilde = ppr->Omega0_cdm_min_synchronous;
  ...
  if (N_ncdm > 0){
    ...
    for(n=0; n<N_ncdm; n++){
      if (pba->M_ncdm[n]!=0.0){
        ...
        /* Complete set of parameters */
        pba->Omega0_ncdm[n] = pba->M_ncdm[n]/pow(0.7,2);
      }
      /* Set default value
    ...
    for (n=0; n < N_ncdm; n++){
      if (pba->m_ncdm_in_eV[n] != 0.0){
        ...
        if (pba->Omega0_ncdm[n] == 0.0){
          pba->Omega0_ncdm[n] = rho_ncdm/(70000/_c_);
        }
        else{
          fnu_factor = ((70000/_c_)*pba->Omega0_ncdm[n]/rho_ncdm);
  ...
  /* Complete set of parameters */
  pba->K = -pba->Omega0_k*pow((70000/_c_),2);
  if (pba->K > 0.){
    pba->sgnK = 1;
  }
  else if (pba->K < 0.){
  ...
  class_read_double("beta_4",pba->beta_4);
  class_read_double("beta_5",pba->beta_5);
  class_read_double("beta_6",pba->beta_6);
  class_read_double("beta_7",pba->beta_7);
  ...
  if (flag1 == _TRUE_)
    ...
    pba->phi_ini_scf = 0.0000000000001;

  class_call(parser_read_double(pfc,"phi_prime_ini_scf",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  ...
  if (flag1 == _TRUE_)
    ...
    pba->phi_prime_ini_scf = 50*sqrt(6)*pba->beta_7*pba->Omega_cdm_tilde*pow(0.7,2)*
                  exp(-sqrt(6)*pba->beta_7*pba->phi_ini_scf/sqrt(2))/sqrt(2)/(299792.458*sqrt((pba->Omega_g_tilde+pba->Omega_ur_tilde)*pow(0.7,2)));
 ...
 /**/
  ...
  if (flag2 == _TRUE_){
    pba->Omega0_dcdmdr = param2/pow(0.7,2);
  }
  ...
  if (pba->Omega0_dcdmdr > 0) {
    ...
    if (flag2 == _TRUE_){
      pba->Omega_ini_dcdm = param2/pow(0.7,2);
    }
  ...
  if (flag2 == _TRUE_)
    pba->Omega0_idm_dr = param2/pow(0.7,2);
  ...
  if (flag3 == _TRUE_) {
    ...
    pba->Omega0_idm_dr = param3 * pba->Omega_cdm_tilde;
    /* readjust Omega0_cdm */
    pba->Omega_cdm_tilde -= pba->Omega0_idm_dr;
    /* avoid Omega0_cdm =0 in synchronous gauge */
    if ((ppt->gauge == synchronous) && (pba->Omega_cdm_tilde==0)) {
      pba->Omega_cdm_tilde += ppr->Omega0_cdm_min_synchronous;
  ...
  if (has_m_budget == _TRUE_) {
    pba->Omega_cdm_tilde = Omega_m_remaining;
  }
  ...
  /* Compute Omega_tot */
  Omega_tot = pba->Omega_g_tilde;
  Omega_tot += pba->Omega_b_tilde;
  Omega_tot += pba->Omega_ur_tilde;
  Omega_tot += pba->Omega_cdm_tilde;
...
int input_read_parameters_spectra(struct file_content * pfc,
  ...
  if ((ppt->has_pk_matter == _TRUE_) || (ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_)){
    ...
    /* Complete set of parameters */
    if (flag1 == _TRUE_){
      ppm->k_max_for_primordial_pk=param1*0.7;
...
int input_default_params(struct background *pba,
  ...
  /* NEW */
  pba->kappa = 0.7;
  pba->beta_1 = 0.0000001;
  pba->beta_2 = 0.0000001;
  pba->beta_3 = 0.0000001;
  pba->beta_4 = 0.0000001;
  pba->beta_5 = 0.0000001;
  pba->beta_6 = 0.0000001;
  pba->beta_7 = 0.0000001;
