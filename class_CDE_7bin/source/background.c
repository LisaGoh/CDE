...
int background_functions(
  ...
  /* photons */
  /* NEW */
  pvecback[pba->index_bg_rho_g] = pba->Omega_g_tilde * pow(70000/_c_,2) / pow(a,4);
  ...
  /* baryons */
  /* NEW */
  pvecback[pba->index_bg_rho_b] = pba->Omega_b_tilde * pow(70000/_c_,2) / pow(a,3);
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    pvecback[pba->index_bg_beta] = beta(pba,a); // coupling beta
    pvecback[pba->index_bg_beta_prime] = beta_prime(pba,a); // Derivative of the coupling beta wrt the redshift z
    /* */
  ...
  if (pba->has_lambda == _TRUE_) {
    pvecback[pba->index_bg_rho_lambda] = pba->Omega0_lambda * pow((70000/_c_),2);
  ...
  if (pba->has_ur == _TRUE_) {
    /* NEW */
    pvecback[pba->index_bg_rho_ur] = pba->Omega_ur_tilde * pow(70000/_c_,2) / pow(a,4);
  ...
  if (pba->has_idm_dr == _TRUE_) {
    pvecback[pba->index_bg_rho_idm_dr] = pba->Omega0_idm_dr * pow(70000/_c_,2) / pow(a,3);
  ...
  if (pba->has_idr == _TRUE_) {
    pvecback[pba->index_bg_rho_idr] = pba->Omega0_idr * pow(70000/_c_,2) / pow(a,4);
...
int background_w_fld(
  ...
  case EDE:
    ...
    Omega_r = pba->Omega_g_tilde * (1. + 3.044 * 7./8.*pow(4./11.,4./3.)); // assumes LambdaCDM + eventually massive neutrinos so light that they are relativistic at equality; needs to be generalised later on.
    Omega_m = pba->Omega0_b;
    if (pba->has_cdm == _TRUE_) Omega_m += pba->Omega_cdm_tilde;
...
/* */
  ...
  /*    */
   /* NEW */
  class_define_index(pba->index_bg_beta,pba->has_scf,index_bg,1);
  class_define_index(pba->index_bg_beta_prime,pba->has_scf,index_bg,1);
  /* */
  /*    */
...
int background_ncdm_M_from_Omega(
  ...
  rho0 = (70000/_c_)*(70000/_c_)*pba->Omega0_ncdm[n_ncdm]; /*Remember that rho is defined such that H^2=sum(rho_i) */
  M = 0.0;
...
int background_checks(
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for (n_ncdm=0;n_ncdm<pba->N_ncdm; n_ncdm++) {
        ...
        printf(" -> non-cold dark matter species with i=%d has m_i = %e eV (so m_i / omega_i =%e eV)\n",
               ...
               pba->m_ncdm_in_eV[n_ncdm]*pba->deg_ncdm[n_ncdm]/pba->Omega0_ncdm[n_ncdm]/pow(0.7,2));
    ...
    if (pba->has_idr == _TRUE_) {
      N_dark = pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega_g_tilde;
...
int background_initial_conditions(
  ...
  */
  ...
  Omega_rad = pba->Omega_g_tilde;
  if (pba->has_ur == _TRUE_) {
    Omega_rad += pba->Omega_ur_tilde;
  }
  if (pba->has_idr == _TRUE_) {
    Omega_rad += pba->Omega0_idr;
  }
  rho_rad = Omega_rad*pow(70000/_c_,2)/pow(a,4);
  if (pba->has_ncdm == _TRUE_) {
  ...
  if (pba->has_dcdm == _TRUE_) {
    ...
    pvecback_integration[pba->index_bi_rho_dcdm] =
      pba->Omega_ini_dcdm*(70000/_c_)*(70000/_c_)*pow(a,-3);
  ...
  if (pba->has_dr == _TRUE_) {
    if (pba->has_dcdm == _TRUE_) {
       ...
       *
       * `f = -Omega_rad+pow(pow(Omega_rad,3./2.)+0.5*pow(a,6)*pvecback_integration[pba->index_bi_rho_dcdm]*pba->Gamma_dcdm/pow((70000/_c_)(70000/_c_),3),2./3.);`
       *
       ...
       */
      f = 1./3.*pow(a,6)*pvecback_integration[pba->index_bi_rho_dcdm]*pba->Gamma_dcdm/pow((70000/_c_),3)/sqrt(Omega_rad);
      pvecback_integration[pba->index_bi_rho_dr] = f*(70000/_c_)*(70000/_c_)/pow(a,4);
    }
    else{
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    /* rho_fld today */
    rho_fld_today = pba->Omega0_fld * pow((70000/_c_),2);
  ...
  if (pba->has_cdm == _TRUE_){
    pvecback_integration[pba->index_bi_rho_cdm] =
      pba->Omega_cdm_tilde*pow(70000/_c_,2)*pow(1/a,3);
  }
  /* */
...
int background_output_titles(
  ...
  class_store_columntitle(titles,"(.)Om",pba->has_scf); 
  class_store_columntitle(titles,"(.)Our",pba->has_scf); 
  class_store_columntitle(titles,"(.)Oscf",pba->has_scf); 
  class_store_columntitle(titles,"(.)OscfKin",pba->has_scf); 
  class_store_columntitle(titles,"(.)Otot",pba->has_scf); 
  /**/
  ...
  /** NEW */
  class_store_columntitle(titles,"beta",_TRUE_);
  class_store_columntitle(titles,"dbeta/dz",_TRUE_);
  /* */
  return _SUCCESS_;
}
...
int background_output_data(
  ...
  /* NEW */
    class_store_double(dataptr,pvecback[pba->index_bg_p_scf]/pvecback[pba->index_bg_rho_scf],pba->has_scf,storeidx);
    class_store_double(dataptr,(pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_b])/pow(pvecback[pba->index_bg_H],2),pba->has_scf,storeidx);
    class_store_double(dataptr,(pvecback[pba->index_bg_rho_g]+pvecback[pba->index_bg_rho_ur])/pow(pvecback[pba->index_bg_H],2),pba->has_scf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_rho_scf]/pow(pvecback[pba->index_bg_H],2),pba->has_scf,storeidx);
    class_store_double(dataptr,pow(pvecback[pba->index_bg_phi_prime_scf],2)/(2*pow(pvecback[pba->index_bg_a],2))/pow(pvecback[pba->index_bg_H],2),pba->has_scf,storeidx);
    class_store_double(dataptr,(pvecback[pba->index_bg_rho_g]+pvecback[pba->index_bg_rho_ur]+pvecback[pba->index_bg_rho_scf]+pvecback[pba->index_bg_rho_cdm]+pvecback[pba->index_bg_rho_b])/pow(pvecback[pba->index_bg_H],2),pba->has_scf,storeidx);
    /* */
    ...
    /** NEW */
    class_store_double(dataptr,pvecback[pba->index_bg_beta],_TRUE_,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_beta_prime],_TRUE_,storeidx);
    /* */
  }
...
int background_derivs(
  ...
  struct background * pba;
  double * pvecback, a, H, rho_M, scf_beta;

  pbpaw = parameters_and_workspace;
  pba =  pbpaw->pba;
  pvecback = pbpaw->pvecback;
  scf_beta = pvecback[pba->index_bg_beta];
  ...
  dy[pba->index_bi_D_prime] = (-a*H+pow(6,0.5)*scf_beta*y[pba->index_bi_phi_prime_scf])*y[pba->index_bi_D_prime]/(a*H) + 1.5*a*rho_M*y[pba->index_bi_D]*(1+4*pow(scf_beta,2))/H;
  /* */
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    /* NEW */
    if (pba->has_cdm == _TRUE_){
    	/** - Conservation equation for the DM */
    dy[pba->index_bi_rho_cdm] = -3.*y[pba->index_bi_rho_cdm]-
      pow(6,0.5)*scf_beta*y[pba->index_bi_rho_cdm]*y[pba->index_bi_phi_prime_scf]/a/H;
  }
  /* */
  ...
  if (pba->has_scf == _TRUE_){
    ...
    dy[pba->index_bi_phi_prime_scf] = -(2*H*y[pba->index_bi_phi_prime_scf]
       + a*dV_scf(pba,y[pba->index_bi_phi_scf]))/H + scf_beta*pow(6,0.5)*pvecback[pba->index_bg_rho_cdm]*pow(a,2)/a/H;
  }
  /* */
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      budget_matter+=pba->Omega_cdm_tilde;
    }
    if (pba->has_idm_dr == _TRUE_) {
    ...
    if (pba->N_ncdm > 0) {
      for(index_ncdm=0;index_ncdm<pba->N_ncdm;++index_ncdm) {
        printf("-> %-26s%-4d Omega = %-15g , omega = %-15g\n","Non-Cold Species Nr.",index_ncdm+1,pba->Omega0_ncdm[index_ncdm],pba->Omega0_ncdm[index_ncdm]*pow(0.7,2));
    ...
    budget_radiation+=pba->Omega_g_tilde;
    if (pba->has_ur == _TRUE_) {
      class_print_species("Ultra-relativistic relics",ur);
      budget_radiation+=pba->Omega_ur_tilde;
    }
    if (pba->has_dr == _TRUE_) {
    ...
    printf(" Radiation                        Omega = %-15g , omega = %-15g \n",budget_radiation,budget_radiation*pow(0.7,2));
    printf(" Non-relativistic                 Omega = %-15g , omega = %-15g \n",budget_matter,budget_matter*pow(0.7,2));
    if (pba->N_ncdm > 0) {
      printf(" - Non-Free-Streaming Matter      Omega = %-15g , omega = %-15g \n",pba->Omega0_nfsm,pba->Omega0_nfsm*pow(0.7,2));
      printf(" - Non-Cold Dark Matter           Omega = %-15g , omega = %-15g \n",budget_neutrino,budget_neutrino*pow(0.7,2));
    }
    if ((pba->has_lambda == _TRUE_) || (pba->has_fld == _TRUE_) || (pba->has_scf == _TRUE_) || (pba->has_curvature == _TRUE_)) {
      printf(" Other Content                    Omega = %-15g , omega = %-15g \n",budget_other,budget_other*pow(0.7,2));
    }
    printf(" TOTAL                            Omega = %-15g , omega = %-15g \n",budget_radiation+budget_matter+budget_other,(budget_radiation+budget_matter+budget_other)*pow(0.7,2));
    printf(" -------------------------------------------------------------------- \n");
  }
  return _SUCCESS_;
}
...
/**
 * Scalar field potential: Peebles-Ratra
 */
...
double beta(
  ...
  //split beta into 10 tomographic bins 
    
    double a = pba->beta_1/sqrt(2);
    double b = pba->beta_2/sqrt(2);
    double c = pba->beta_3/sqrt(2);
    double d = pba->beta_4/sqrt(2);
    double e = pba->beta_5/sqrt(2);
    double f = pba->beta_6/sqrt(2);
    double g = pba->beta_7/sqrt(2);

    double s_1 = 10;
    double s_2 = 0.03;
  
  return (g-f)*tanh(s_2*(z-1000))/2
         +(f-e)*tanh(s_2*(z-500))/2
         +(e-d)*tanh(s_2*(z-100))/2
         +(d-c)*tanh(s_1*(z-5))/2
         +(c-b)*tanh(s_1*(z-2))/2
         +(b-a)*tanh(s_1*(z-1))/2
         +(g+a)/2;
...
/* This prime denotes a derivative with respect to the redshift */
...
double beta_prime(
  ...
  return s_2*((g-f)/(2*cosh(s_2*(z-1000))*cosh(s_2*(z-1000)))
             +(f-e)/(2*cosh(s_2*(z-500))*cosh(s_2*(z-500)))
             +(e-d)/(2*cosh(s_2*(z-100))*cosh(s_2*(z-100))))
             +s_1*((d-c)/(2*cosh(s_1*(z-5))*cosh(s_1*(z-5)))
            +(c-b)/(2*cosh(s_1*(z-2))*cosh(s_1*(z-2)))
            +(b-a)/(2*cosh(s_1*(z-1))*cosh(s_1*(z-1))));
